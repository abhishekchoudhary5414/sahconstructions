...
export default function CallDashboard() {
  ...
  useEffect(() => {
    ...
    async function loadCalls() {
      ...
      try {
        const response = await fetch(`/api/admin/calls?${params.toString()}`, { signal: controller.signal });
        if (!response.ok) throw new Error('Unable to fetch call clicks.');
        ...
        if (!(err instanceof DOMException && err.name === 'AbortError')) setError('Unable to load call clicks. Refresh the page and try again.');
      } finally {
        setLoading(false);
      }
    }
    ...
    <section className={styles.callSection}>
      <div className={styles.dashboardHeader}>
        <div>
          ...
          <h2>Track call engagement</h2>
          <p className={styles.subText}>Review recent call clicks and understand which pages generate the most customer calls.</p>
        </div>
        <div className={styles.statsRow}>
          <div className={`${styles.statCard} ${styles.totalCard}`}><strong>{totalRecords}</strong><span>Total calls</span></div>
        ...
        <div className={styles.topPageSection}>
          ...
          <span className={styles.topPageCount}>{topPageCount} calls</span>
          <span className={styles.topPageHint}>Most calls in the selected range</span>
        </div>
      </div>

      <div className={styles.controlsRow}>
        <div className={styles.controlsLabel}>Showing {clicks.length} / {totalRecords} calls{pageSize !== 0 && ` • Page ${page} of ${pageCount}`}</div>
        <div className={styles.controls}>
          <label className={styles.controlGroup}><span>Range</span><select className={styles.controlSelect} value={range} onChange={(event) => { setRange(event.target.value); setPage(1); }}>{ranges.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
      ...
      <div className={styles.tableContainer}>
        <table className={styles.table}>
          ...
          <tbody>
            {loading ? <tr><td colSpan={5} className={styles.loadingRow}><div className={styles.skeletonRows} role="status" aria-label="Loading call clicks">{Array.from({ length: 6 }).map((_, index) => <div className={styles.skeletonLine} key={index} />)}</div></td></tr> : clicks.length === 0 ? <tr><td colSpan={5} className={styles.emptyRow}>No call clicks found.</td></tr> : clicks.map((click, index) => <tr key={click.id}><td>{pageSize === 0 ? index + 1 : (page - 1) * pageSize + index + 1}</td><td className={styles.pageCell}>{click.page_url || '-'}</td><td>{click.browser || '-'}</td><td className={styles.userAgentCell}>{click.user_agent || '-'}</td><td>{new Date(click.created_at).toLocaleString()}</td></tr>)}
          </tbody>
        </table>
      </div>
