...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const cityTitle = city ? `${city.city}, ${city.state}, ${city.country}` : '';
  const canonicalSlug = city ? normalizeCitySlug(city.city) : '';

  return {
    title: cityTitle ? `${work.title} in ${cityTitle} | SAH Constructions` : `${work.title} | SAH Constructions`,
    description: cityTitle
      ? `${work.summary} SAH Constructions is delivering ${work.title.toLowerCase()} across ${cityTitle}.`
      : work.summary,
    alternates: {
      canonical: city && canonicalSlug
        ? `https://www.sahconstructions.com/works/${work.slug}/${canonicalSlug}`
        : `https://www.sahconstructions.com/works/${work.slug}`
    }
  };
}
