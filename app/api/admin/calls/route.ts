...
export async function GET(request: Request) {
  ...
  let query = supabase.from('call_clicks').select('*', { count: 'exact' });
  let pageQuery = supabase.from('call_clicks').select('page_url');
  if (fromDate) {
  ...
  return Response.json({ data, count, topPage: topPageEntry?.[0] || null, topPageCount: topPageEntry?.[1] || 0 });
}
